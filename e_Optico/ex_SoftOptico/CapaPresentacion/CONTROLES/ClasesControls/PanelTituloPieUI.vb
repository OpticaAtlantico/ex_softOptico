...
Public Class PanelTituloPieUI
    ...
    Private _borderColorTitulo As Color = AppColors._cBasePrimary
...
#Region "DIBUJO"
    ...
    Private Function RoundedPiePath(rect As Rectangle, radius As Integer) As GraphicsPath
        Dim path As New GraphicsPath()
        path.StartFigure()
    ...
    Private Sub Pie_Paint(sender As Object, e As PaintEventArgs) Handles Pie.Paint
        Dim pnl As Panel = DirectCast(sender, Panel)
        e.Graphics.SmoothingMode = SmoothingMode.AntiAlias

        Dim rect As New Rectangle(0, 0, pnl.Width, pnl.Height)

        Using path As GraphicsPath = RoundedPiePath(rect, _borderRadius)
            ' 🔹 Asignar la región al panel (recorte físico)
