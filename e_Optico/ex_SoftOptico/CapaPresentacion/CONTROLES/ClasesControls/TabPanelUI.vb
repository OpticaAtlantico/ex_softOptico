...
#Region "DIBUJO"
    ...
    Private Sub DibujarPestañaConIcono(g As Graphics, tab As TabItemOrbitalAdv, rect As Rectangle, isSelected As Boolean)
        ...
        ' Ícono
        Dim textoX As Integer = rect.X + 10
        If tab.Icono <> IconChar.None Then
            Dim ico As IconChar = tab.Icono
            Dim iconImg = IconCharExtensions.ToBitmapPaint(ico, colorTexto, 20)
            g.DrawImage(iconImg, textoX, rect.Y + (rect.Height - iconImg.Height) \ 2)
            textoX += iconImg.Width + 6
        End If

        ' Texto centrado verticalmente
        Dim textoSize = TextRenderer.MeasureText(tab.Titulo, Me.Font)
        Dim textoY = rect.Y + (rect.Height - textoSize.Height) \ 2
        TextRenderer.DrawText(g, tab.Titulo, Me.Font, New Point(textoX, textoY), colorTexto)

        ' Línea inferior activa
        If isSelected Then
            g.FillRectangle(New SolidBrush(Color.FromArgb(220, colorTexto)), rect.X, rect.Bottom - 3, rect.Width, 3)
        End If
    End Sub
#End Region
...
#Region "PROCEDIMIENTOS"
    ...
    Public Sub AvanzarPestaña()
        If currentIndex < Tabs.Count - 1 Then
            SeleccionarPestana(currentIndex + 1)
        End If
    End Sub
